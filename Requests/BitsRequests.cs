...
public static class BitsRequests
{
    ...
    public static async Task<BitsLeaderboardResponseBody> GetBitsLeaderboard(this TwitcherAPI api, string? userId = null, int count = 10, LeaderboardTimePeriod period = LeaderboardTimePeriod.All, DateTime startedAt = default)
    {
        ...
        var response = await api.APIRequest<BitsLeaderboardResponseBody>(request);
    ...
    /// <exception cref="NotValidatedException"></exception>
    /// <exception cref="TwitchErrorException"></exception>
    public static async Task<CheermotesResponseBody[]> GetCheermotes(this TwitcherAPI api, string? broadcasterId = null)
    {
        ...
        var response = await api.APIRequest<DataResponse<CheermotesResponseBody[]>>(request);
